...
def pred_patch(model):
    ...
    naip_data, padding, dst_transformation_data = DataLoader.get_data_by_extent(naip_fn, extent, DataLoader.GeoDataTypes.NAIP, return_transforms=True)
    ...
    transforms.append(dst_transformation_data)
...
def main():
    ...
    parser.add_argument("--model", action="store", dest="model", choices=["kdd", "old_cached", "new_cached", "iclr", "mila"], help="Model to use", required=True)
    ...
    if args.model == "old_cached":
        ...
        loaded_model = ServerModelsKDD.run
    else:
